...
if '-DRADIO_SX127X=1' in build_flags or '-DRADIO_LR1121=1' in build_flags:
    ...
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_US_433_WIDE'):
        json_flags['domain'] = 7
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_550_650_100'):
        json_flags['domain'] = 8
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_380_440_60'):
        json_flags['domain'] = 9
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_200_210_10'):
        json_flags['domain'] = 10
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_C170W23'):
        json_flags['domain'] = 11
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_C190W23'):
        json_flags['domain'] = 12
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_470_520_50'):
        json_flags['domain'] = 13
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_450_550_100'):
        json_flags['domain'] = 14
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_490_560_70'):
        json_flags['domain'] = 15
    if fnmatch.filter(build_flags, '*-DRegulatory_Domain_UA_610_650_40'):
